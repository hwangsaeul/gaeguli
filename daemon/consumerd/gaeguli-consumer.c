...
struct pollfd fds[GAEGULI_MAX_CONSUMERS + 1];
int numfds = 0;
...
_stop_server ()
{
  unlink (DEFAULT_CONSUMER_CLIENT_SOCK_PATH);
  syslog (LOG_INFO, "Killing gaeguli_consumer daemon with PID: %ld",
      (long) getpid ());
  kill (0, SIGKILL);
  exit (0);
}
...
_handle_connection (int client_sockfd)
{
  ...
  gst_init (NULL, NULL);        /* Init the gst ebvironment here */
    ...
    for (fd_idx = 0; fd_idx < numfds; fd_idx++) {
      if (fds[fd_idx].revents & (POLLERR | POLLHUP)) {
        ...
        if (fds[fd_idx].fd == client_sockfd) {
          ...
          if (len > 0) {
            msg = (GaeguliConsumerMsg *) data;
            rsp = gaeguli_process_consumer_message (msg);
...
main (int argc, char *argv[])
{
  ...
  int limit = GAEGULI_MAX_CONSUMERS + 1;
  int res = EXIT_FAILURE;

  gaeguli_daemonize ("gaeguli_consumer");

  signal (SIGCHLD, _handle_child);
  signal (SIGTERM, _stop_server);

  if ((server_sockfd =
          gaeguli_init_socket (DEFAULT_CONSUMER_CLIENT_SOCK_PATH)) < 0) {
  ...
  syslog (LOG_INFO, "gaeguli_consumer daemon is ready. Listening ...");

  while (1) {
    pid_t pid;

    syslog (LOG_INFO,
        "gaeguli_consumer daemon is waiting for a connection ...");
    ...
    syslog (LOG_INFO,
        "gaeguli_consumer daemon accepted a new connection where client_sockfd = %d, server_sockfd = %d",
...
out:
  syslog (LOG_INFO, "gaeguli_consumer daemon terminated.");
  close (server_sockfd);
  close (client_sockfd);
  closelog ();
  return res;
